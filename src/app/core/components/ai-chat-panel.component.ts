...
/**
 * F20.1-b — panel de conversación del asistente IA extraído de
 * `ChatbotComponent` (F20.1) para reutilizarlo tanto en la pantalla
 * dedicada `/chatbot` como en el widget flotante global
 * (`ChatWidgetComponent`). Ambos consumidores muestran la MISMA
 * conversación: el backend guarda una sola conversación activa por
 * usuario (`AiChatHistory.conversationId` es singular), así que cada
 * instancia de este panel carga su propio historial en `ngOnInit` y
 * ambas terminan viendo los mismos mensajes sin necesidad de estado
 * compartido entre instancias.
 *
 * Este componente es el único dueño del estado y la lógica del chat
 * (mensajes, envío, feedback, links) — la página `/chatbot` y el widget
 * son solo distintos contenedores visuales alrededor de la misma
 * instancia de lógica, para no duplicarla (criterio de aceptación de
 * HU-F20-1-b).
 */
@Component({
      ...
      <header class="flex items-center justify-between rounded-2xl bg-surface-muted px-4 py-3">
        ...
        <div class="flex items-center gap-2">
          ...
          @if (showCloseButton) {
            <button
              type="button"
              class="rounded-full p-1 text-text-subtle transition hover:bg-surface hover:text-text"
...
export class AiChatPanelComponent implements OnInit {
   ...
   * pantalla `/chatbot` no lo necesita (ya tiene su propia navegación). */
