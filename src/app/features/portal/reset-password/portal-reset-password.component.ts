  ...
  template: `
    <div class="flex min-h-screen items-center justify-center bg-surface-muted px-6 py-12">
      <div class="w-full max-w-md rounded-lg border border-default bg-white/80 p-8 shadow-raised backdrop-blur">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-text">Restablecer contraseña</h1>
...
export class PortalResetPasswordComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.portalAuthService.resetPassword(token, newPassword).subscribe({
      next: (result) => {
        this.isSubmitting.set(false);
        if (result.success) {
          this.success.set(true);
          setTimeout(() => this.router.navigateByUrl('/portal/login'), 1500);
        } else {
