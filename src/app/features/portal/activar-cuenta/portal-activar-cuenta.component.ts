...
@Component({
  selector: 'app-portal-activar-cuenta',
  standalone: true,
  ...
  template: `
    <div class="flex min-h-screen items-center justify-center bg-surface-muted px-6 py-12">
      <div class="w-full max-w-md rounded-lg border border-default bg-white/80 p-8 shadow-raised backdrop-blur">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-text">Activa tu acceso</h1>
        <p class="mt-2 text-sm text-subtle">Crea tu contraseña para entrar a tu portal.</p>

        @if (!token()) {
          <div class="mt-8 rounded-md border border-danger bg-danger-tint px-4 py-3 text-sm text-danger">
            El enlace no incluye un token válido. Pide al despacho que reenvíe la invitación.
          </div>
        } @else if (success()) {
          <div class="mt-8 rounded-md border border-default bg-surface px-4 py-3 text-sm text-text">
            Tu acceso fue activado. Estamos redirigiéndote a tu portal...
          </div>
        } @else {
...
export class PortalActivarCuentaComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.portalAuthService.acceptInvitation(token, password).subscribe({
      next: (result) => {
        this.isSubmitting.set(false);
        if (result.success) {
          this.success.set(true);
          setTimeout(() => this.router.navigateByUrl('/portal/procesos'), 1500);
        } else {
