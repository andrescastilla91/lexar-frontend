...
/**
 * F40 Ola 4a — ajuste 2026-09-21 (feedback punto 2: el formulario "Nuevo
 * plazo" quedaba siempre visible encima del listado, obligando a hacer
 * scroll y quitándole prioridad visual a "Plazos registrados"). En modo
 * `embedded` el formulario ahora vive oculto detrás del botón "+ Nuevo
 * plazo" (mismo patrón que ClientMattersPanelComponent en F34 y el ajuste
 * equivalente hecho en ProcessCounterpartiesModalComponent), y el listado
 * queda siempre visible sin envoltorio de scroll propio — la página ya
 * scrollea. El modo no-embedded (overlay `fixed inset-0`, sin call-sites
 * reales tras Ola 4a) conserva el formulario siempre visible, sin cambios.
 */
@Component({
  ...
  template: `
    ...
    <!-- F40 Ola 4a: diálogo flotante de alta en modo embedded — el listado
         de arriba se mantiene siempre visible mientras este está abierto. -->
    @if (embedded() && formOpen()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-lg overflow-y-auto rounded-lg border border-default bg-surface p-6 shadow-2xl max-h-[90vh]">
          <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-text">Nuevo plazo</h3>
            <button
              type="button"
...
export class ProcessDeadlinesModalComponent {
  isOpen = input(false);
  /** F40 Ola 4a: true cuando se renderiza como panel de la pestaña "Plazos"
  ...
  /** F40 Ola 4a: estado puramente visual (qué panel se muestra) del modo
   * embedded — igual que `activeTab` en ProcessCounterpartiesModalComponent,
   * no es dato de negocio, así que vive local al componente "tonto". */
  readonly formOpen = signal(false);
  private readonly wasSubmitting = signal(false);

  constructor() {
    // Cierra el diálogo de alta automáticamente cuando el padre reporta que
    // terminó de enviar sin error (mismo efecto que el save() de
    // ClientMattersPanelComponent, adaptado a un componente input/output).
    effect(() => {
