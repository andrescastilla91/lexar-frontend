 ...
 * F14-tareas-workflows.md). Reestructurado de nuevo 2026-09-21 (F40 Ola 4a,
 * feedback punto 3 "mala distribución visual" ya embebido en
 * ProcessDetailComponent): el tab-strip interno "Listado | Nueva tarea |
 * Desde plantilla" duplicaba visualmente la barra de pestañas de la ficha
 * de proceso que queda justo encima. En modo `embedded` se reemplaza por el
 * mismo header título+acciones de `ClientMattersPanelComponent` (F34): el
 * listado queda siempre visible y "Nueva tarea"/"Desde plantilla" abren
 * cada uno su propio diálogo flotante. El modo no-embedded (overlay
 * `fixed inset-0`, sin call-sites reales tras Ola 4a) conserva el
 * tab-strip original, sin cambios.
 *
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, TaskStatusControlComponent, TaskEditModalComponent, NgTemplateOutlet],
  ...
  template: `
      ...
      <div [class]="embedded() ? '' : 'fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4'">
        <div
          [class]="embedded()
            ...
            : 'w-full max-w-xl md:max-w-2xl lg:max-w-3xl flex flex-col rounded-lg border border-default bg-surface shadow-2xl overflow-hidden max-h-[90vh]'"
        >
          @if (!embedded()) {
            <!-- Header -->
            ...
            <div class="flex-1 overflow-y-auto p-6">
              @switch (activeTab()) {
                @case ('new') {
                  <ng-container *ngTemplateOutlet="newFormTpl" />
                }
                @case ('template') {
                  <ng-container *ngTemplateOutlet="templateFormTpl" />
                }
                @default {
            ...
            <div class="flex items-center justify-between border-b border-default p-4">
              <h3 class="text-sm font-semibold text-text">Tareas ({{ tasks().length }})</h3>
              <div class="flex gap-2">
                @if (templates().length > 0) {
                  <button
                    type="button"
                    (click)="templateFormOpen.set(true)"
                    class="rounded-md border border-default px-3 py-2 text-xs font-semibold text-muted transition hover:bg-surface-muted"
                  >
                    Desde plantilla
                  </button>
                }
                <button
                  type="button"
                  (click)="newTaskFormOpen.set(true)"
    ...
    <!-- F40 Ola 4a: diálogos flotantes de alta en modo embedded — el listado
         de arriba se mantiene siempre visible mientras están abiertos. -->
    @if (embedded() && newTaskFormOpen()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-lg overflow-y-auto rounded-lg border border-default bg-surface p-6 shadow-2xl max-h-[90vh]">
          <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-text">Nueva tarea</h3>
            <button
              type="button"
              (click)="newTaskFormOpen.set(false)"
              class="rounded-md p-2 text-subtle hover:bg-surface-muted hover:text-muted"
            >
              <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
    ...
    @if (embedded() && templateFormOpen()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div class="w-full max-w-lg overflow-y-auto rounded-lg border border-default bg-surface p-6 shadow-2xl max-h-[90vh]">
          <div class="mb-4 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-text">Crear tareas desde plantilla</h3>
            <button
              type="button"
              (click)="templateFormOpen.set(false)"
              class="rounded-md p-2 text-subtle hover:bg-surface-muted hover:text-muted"
            >
              <svg class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
...
export class ProcessTasksModalComponent {
  isOpen = input(false);
  /** F40 Ola 4a: true cuando se renderiza como panel de la pestaña "Tareas"
   * en ProcessDetailComponent, en vez de overlay modal. */
  ...
  /** F40 Ola 4a: estado puramente visual del modo embedded — qué diálogo
   * flotante está abierto sobre el listado, siempre visible detrás. */
  readonly newTaskFormOpen = signal(false);
  readonly templateFormOpen = signal(false);
  private readonly wasSubmitting = signal(false);
  ...
  constructor() {
    // Cierra el diálogo de "Nueva tarea" automáticamente cuando el padre
    // reporta que terminó de enviar sin error, igual que en
    // ProcessDeadlinesModalComponent. "Desde plantilla" se cierra solo
    // manualmente: permite instanciar varias plantillas seguidas.
    effect(() => {
      const submitting = this.isSubmitting();
      if (this.wasSubmitting() && !submitting && !this.errorMessage()) {
        this.newTaskFormOpen.set(false);
      }
      this.wasSubmitting.set(submitting);
    });
  }
