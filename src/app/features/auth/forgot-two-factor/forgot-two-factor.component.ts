...
@Component({
  selector: 'app-forgot-two-factor',
  standalone: true,
  ...
  template: `
    <div class="flex min-h-screen items-center justify-center bg-surface-muted px-6 py-12">
      <div class="w-full max-w-md rounded-lg border border-default bg-white/80 p-8 shadow-raised backdrop-blur">
        <h1 class="text-2xl font-semibold text-text">¿Perdiste acceso a tu verificación en dos pasos?</h1>
        <p class="mt-2 text-sm text-subtle">
          Ingresa tu correo. Si tienes la verificación en dos pasos activa, avisaremos a los administradores de tu empresa para que revisen tu solicitud y la completen tras verificar tu identidad — por seguridad, esto no se resuelve automáticamente por correo.
        </p>

        @if (submitted()) {
          <div class="mt-8 rounded-md border border-default bg-surface px-4 py-3 text-sm text-text">
            Si el correo existe y tiene la verificación en dos pasos activa, notificamos a los administradores de tu empresa. Contacta a tu administrador si necesitas agilizar el proceso.
          </div>
        } @else {
...
export class ForgotTwoFactorComponent {
  ...
  onSubmit(): void {
    ...
    this.authService.forgotTwoFactor(email).subscribe({
      next: (result) => {
