  ...
  template: `
    <div class="flex min-h-screen items-center justify-center bg-surface-muted px-6 py-12">
      <div class="w-full max-w-md rounded-lg border border-default bg-white/80 p-8 shadow-raised backdrop-blur">
        <h1 class="text-2xl font-semibold text-text">Restablecer contraseña</h1>
        <p class="mt-2 text-sm text-subtle">Ingresa tu nueva contraseña.</p>
        ...
        @if (!token()) {
          ...
          <div class="mt-8 rounded-md border border-default bg-surface px-4 py-3 text-sm text-text">
            Tu contraseña fue restablecida. Ya puedes iniciar sesión con ella.
          </div>
        } @else {
          <form class="mt-8 space-y-6" [formGroup]="form" (ngSubmit)="onSubmit()">
            <div class="space-y-2">
              <label class="block text-sm font-medium text-muted" for="newPassword">Nueva contraseña</label>
              <input
                id="newPassword"
                type="password"
